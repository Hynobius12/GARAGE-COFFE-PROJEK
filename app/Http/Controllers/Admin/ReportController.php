...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        // POS orders list - Hapus ->map(...)
        $posOrders = Order::where('status', 'completed')
            ->whereBetween('created_at', [$startDate . ' 00:00:00', $endDate . ' 23:59:59'])
            ->latest()->get();

        // Web orders list - Hapus ->map(...)
        $webOrders = QrisOrder::where('status', 'completed')
            ->whereBetween('created_at', [$startDate . ' 00:00:00', $endDate . ' 23:59:59'])
            ->latest()->get();

        // Gabungkan keduanya (Data tetap berupa Object Model)
        $orders = $posOrders->merge($webOrders)->sortByDesc('created_at')->values();
    ...
    public function exportPdf(Request $request)
    {
        ...
        // Ambil data tanpa dimapping menjadi array
        $orders = $posOrders->latest()->get()
            ->merge($webOrders->latest()->get())
